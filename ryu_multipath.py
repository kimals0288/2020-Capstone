...
from ryu.topology import event as tpevent 
...
class ProjectController(app_manager.RyuApp):
    ...
    def start(self):
        super(ProjectController, self).start()
    ...
    @set_ev_cls(TestEvent)
    def update_resources(self, ev):
      print ("DROP RULE CREATE")
      ...
      for curSwitch in self.datapaths:
        ...
        if curSwitch.id == 2:
          #####in_port=2,src=10.0.0.3,dst=10.0.0.1---> drop
          ofproto = datapath.ofproto
          parser = datapath.ofproto_parser
          match = parser.OFPMatch(in_port=2, eth_type=0x0800, ipv4_src="10.0.0.3")
          actions = []
          inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, actions)]
          mod = datapath.ofproto_parser.OFPFlowMod(
            datapath=datapath, match=match, cookie=0,
            command=ofproto.OFPFC_ADD, idle_timeout=0, hard_timeout=0,
            priority=4, instructions=inst)
          datapath.send_msg(mod)
        ...
        '''if curSwitch.id == 4:
                                  ...
                                  ofproto = datapath.ofproto
                                  parser = datapath.ofproto_parser
                                  match = parser.OFPMatch(in_port=4, eth_type=0x0800, ipv4_src="10.0.0.3")
                                  actions = []
                                  inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, actions)]
                                  mod = datapath.ofproto_parser.OFPFlowMod(
                                    datapath=datapath, match=match, cookie=0,
    ...
    @set_ev_cls(tpevent.EventSwitchEnter)
